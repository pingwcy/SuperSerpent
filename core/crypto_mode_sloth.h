...
void xts_encrypt(XTS_CTX *ctx, const uint8_t *input, uint8_t *output, size_t length, uint64_t sector_number, uint64_t sector_size);

void xts_decrypt(XTS_CTX *ctx, const uint8_t *input, uint8_t *output, size_t length, uint64_t sector_number, uint64_t sector_size);
#ifdef __cplusplus
}
#endif
